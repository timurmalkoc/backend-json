...
def user_signup():
    ...
    exist_user = User.query.filter((User.email == data["email"]) or (User.username == data["username"])).all()
    print("exist",exist_user)
    if exist_user:
        return jsonify({'error':"Username is token"}),400


    User(email=data['email'], username=data['username'], password=data['password'])
    return jsonify({"success":"New user is created successfully !"}),200